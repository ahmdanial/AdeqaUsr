...
<div style="width: 90%; margin-top: 2%; margin-left: auto;" class="container-fluid">
    ...
    <form method="post">
                ...
                <td>
                    <select name="reagent" id="reagent" class="form-control text-center" onchange="this.form.submit()"
                        ...
                        if (isset($_POST['instrument'])) {
                            ...
                            $sqlReagent = "SELECT A.* FROM reagents A, assign_test B WHERE A.id = B.reagent_id 
                            AND A.instrument_id = B.instrument_id AND B.instrument_id = '$instrVal'
                            AND B.lab_id = '$labID' and B.id = '$instID'";
        ...
        $sqlEntry = "SELECT A.* FROM tests A, subassigntest B, assign_test C WHERE A.reagent_id = '$reagentValue'
    ...
    if (isset($_POST['reagent'])) {
        ...
        $sqlDate3 = "SELECT * FROM entryresults A, assign_test B, programs C WHERE A.assignTestID = B.id AND 
                C.id = B.prog_id AND B.prog_id = '$programID'";

        $numRowDate = $conn->query($sqlDate3)->num_rows;

        $sqlDate = "SELECT E.testcode, A.unit, C.methodname, D.testname, B.ID, AA.result_1, AA.result_2
                    ...
                    and B.id = E.assign_test_id and AA.sampledate = '$sampleDate'
                    AND B.prog_id = '$programID' AND B.reagent_id = '$reagentValue' AND B.lab_id = '$labID'
        ...
        if ($numRowDate == 0) {
            ...
            $rowEntry3 = $conn->query($sqlDate)->fetch_object();
            $numRow3 = $conn->query($sqlDate)->num_rows;

            ?>
            <form action="form-submission/formupdate.php" method="post">
                <table class="table table-bordered">
                    <tr style="background-color: #dcdcdc">
                        <th>Test Code</th>
                        <th>Test Name</th>
